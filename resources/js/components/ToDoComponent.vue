    ...
    <div class=" flex w-50">
        <form @submit.prevent="saveData">
            <div class="input-group mb-3 w-100 d-flex">
                ...
                class="form-control form-control-lg" @keydown="form.errors.clear('title')"
                placeholder="" aria-label="To-do title" aria-describedby="button-addon2">
                <button class="btn btn-success" type="submit" id="button-addon2"> Adicione um item para a sua lista </button>
            </div>            
            <span class="text-danger pt-3" v-if="form.errors.has('title')" v-text="form.errors.get('title')"> </span>
        </form> 
        <div class=" flex w-100 todo ">
            <div v-for="todo in todos" :key="todo.id" class="w-100 d-flex align-items-center p-3 bg-white border-bottom">
                ...
                <div class="ml-auto mr-2 d-flex align-items-center grid"> 
                    <span>
                         
                        <svg v-on:click="editmode = todo.id"  v-if="editmode != todo.id" xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-edit justify-self-end" width="36" height="36" viewBox="0 0 24 24" stroke-width="1.5" stroke="#FFC107" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z"/>
                            <path d="M9 7 h-3a2 2 0 0 0 -2 2v9a2 2 0 0 0 2 2h9a2 2 0 0 0 2 -2v-3" />
                            <path d="M9 15h3l8.5 -8.5a1.5 1.5 0 0 0 -3 -3l-8.5 8.5v3" />
                            <line x1="16" y1="5" x2="19" y2="8" />
                        </svg>
                        <svg v-if="editmode == todo.id" v-on:click="updateTodo(todo)"  xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-checkbox justify-self-end" width="36" height="36" viewBox="0 0 24 24" stroke-width="1.5" stroke="#4CAF50" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z"/>
    ...
    export default {
        ...
        methods: {
            ...
            deleteTodo(todo){
                ...
                axios.post('/api/todo/' + todo.id, data).then((response)=>{
                    this.todos = response.data
                }).catch((error)=> {
                    this.form.errors.record(error.response.data.errors);
                })
            ...
            updateTodo(todo){
                ...
                axios.post('/api/todo/' + todo.id,data).catch((error)=>{
                    this.forms.errors.record(error.response.data.errors);
                });
            },
            toggleTodo(todo){
